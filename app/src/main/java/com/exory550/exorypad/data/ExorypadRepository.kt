...
class ExorypadRepository(
    ...
    val savedDraftId: Flow<Long?> = database.noteMetadataQueries.getDraftId()
        .asFlow()
        .mapToList()
        .map { it.firstOrNull() }

    fun noteMetadataFlow(order: SortOrder): Flow<List<NoteMetadata>> = with(database.noteMetadataQueries) {
        when(order) {
            SortOrder.DateDescending -> getSortedByDateDescending().asFlow().mapToList()
            SortOrder.DateAscending -> getSortedByDateAscending().asFlow().mapToList()
            SortOrder.TitleDescending, SortOrder.TitleAscending -> {
                val collator = Collator.getInstance()
                getUnsorted().asFlow().mapToList().map { notes ->
                    val sorted = notes.sortedWith { a, b -> collator.compare(a.title, b.title) }
                    if (order == SortOrder.TitleDescending) sorted.reversed() else sorted
                }
            }
        }
    }
    ...
    suspend fun deleteNote(id: Long, onSuccess: suspend () -> Unit = {}) {
        try {
            with(database) {
    ...
    suspend fun deleteNotes(ids: List<Long>, onSuccess: suspend () -> Unit) {
        try {
            with(database) {
